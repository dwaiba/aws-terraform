...
module "vpc" {
  ...
  secondary_cidr_blocks  = lookup(var.vpc_subnets, "secondary_cidr_blocks")
...
provider "helm" {
  kubernetes {
    config_path = "/home/dbanerjee/aws-terraform/kubeconfig_test-eks"
  }
  #depends_on = [iam_assumable_role_admin]
}
